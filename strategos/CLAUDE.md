...
**Remaining exception:** `usePdsConsolidated` (in `usePdsEntries.ts`) is still a legacy `useState` + `useEffect` + private `tick` hook with **no shared cache** — each mounting component issues its own query, and its `refetch()` refreshes only that component's copy. Migration to React Query is pending. Every other data hook is React Query.
...
|`usePdsConsolidated(planoId?)`                                                               |**Legacy `useState`/`useEffect` hook — NOT React Query, no shared cache.** Merges the 4 PDS item arrays across a plano's `pds_entries` rows. Two components mounting it issue two queries; `refetch()` refreshes only the calling component. Consumed by `GestaoPDS`, `VisaoExecutiva`, `PontoSituacao`. Migration pending.|
