...
Almost all remote state is managed by **React Query** (`@tanstack/react-query`). `QueryClientProvider` is mounted in `main.tsx`. Reference-data hooks (programs, eixos, planos, people, resources, risks, pds entries) use `staleTime: 5min`, `gcTime: 10min`, `retry: 1`, `refetchOnWindowFocus: false`. Activities use `staleTime: 2min`. Financials use a combined 3-table fetch under one key. Snapshots use `staleTime: 15min`. The N+1 fetch pattern (one fetch per plano per page load) is closed.

**Remaining exception:** `usePdsEntries` (in `usePdsEntries.ts`) is still a legacy `useState` + `useEffect` + private `tick` hook with **no shared cache**. This is **deliberate, not an oversight**: it is the read-modify-write source for all three `GestaoPDS` write paths, which rewrite a whole JSONB array column from an in-memory copy with no re-read and no version guard. Today a remount refetches and heals a stale copy; caching it would widen that existing lost-update window. Migrating it belongs with a re-read-before-write (or `updated_at` guard) fix. Every other data hook, including `usePdsConsolidated`, is React Query.
...
|`useResources()`, `useRisks()`, `usePdsEntries()`                                           |Domain data. Each uses React Query with `staleTime: 5min`, `gcTime: 10min`, `retry: 1`, `refetchOnWindowFocus: false`. `useRisks(programId?)` keys on `['risks', programId]` (or `['risks']` when called with no argument — note those are distinct cache entries); `refetch` invalidates the key, which is what refreshes the list after the `GestaoRiscos` create / edit / delete writes.|
|`usePdsConsolidated(planoId?)`                                                               |React Query, key `['pds_consolidated', planoId]`, `enabled` on `planoId` (no plano → no query, empty items). Merges the 4 PDS item arrays across **all** of a plano's `pds_entries` rows: flatten → sort by `created_at` ascending; no de-duplication and no `hidden_at` filtering (each consumer filters itself). `refetch` invalidates the key, which is what refreshes the list after the `GestaoPDS` inline-edit / hide-restore / modal-add writes. Consumed by `GestaoPDS`, `VisaoExecutiva`, `PontoSituacao`. Note: `usePdsEntries` in the same file is intentionally still legacy (see above).|
