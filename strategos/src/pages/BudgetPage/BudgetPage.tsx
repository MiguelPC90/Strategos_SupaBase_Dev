...
export default function BudgetPage() {
  ...
  const aggRows = useMemo((): AggRow[] => {
    ...
    scopedBudget.forEach(b => {
      ...
      const key = `${catName ?? ''}|${isCapex === null ? '' : isCapex ? 'CAPEX' : 'OPEX'}`
      if (!map.has(key)) map.set(key, { key, catName, isCapex, yearTotals: {}, total: 0, subRows: [] })
      const row = map.get(key)!
      const planoId   = b.plano_id ?? ''
      const planoName = planoNameMap.get(planoId) ?? ''
      let sub = row.subRows.find(s => s.planoId === planoId)
      if (!sub) { sub = { planoId, planoName, yearTotals: {}, total: 0 }; row.subRows.push(sub) }
      Object.entries(b.values).forEach(([y, v]) => {
        row.yearTotals[y] = (row.yearTotals[y] ?? 0) + v
        sub!.yearTotals[y] = (sub!.yearTotals[y] ?? 0) + v
        row.total += v
        sub!.total += v
      })
    })
    return Array.from(map.values())
  }, [scopedBudget, planoNameMap])

  const grandBudgetTotal = useMemo(() => aggRows.reduce((s, r) => s + r.total, 0), [aggRows])
  const yearGrandTotals  = useMemo(() => {
    const t: Record<string, number> = {}
    aggRows.forEach(r => budgetYears.forEach(y => { t[y] = (t[y] ?? 0) + (r.yearTotals[y] ?? 0) }))
    return t
  }, [aggRows, budgetYears])
              ...
              <table className="gf-budget-table tbl-default">
                      ...
                      <Fragment key={row.key}>
                        <tr className="bp-agg-row" onClick={toggle}>
                          ...
                          <td style={{ fontWeight: 600 }}>
                            {row.catName ?? <span style={{ color: 'var(--text3)' }}>—</span>}
                          </td>
                          <td className="gf-td-c">
                            {row.isCapex !== null ? (
                              <span style={{
                                ...
                                background: row.isCapex ? '#EBF0FA' : '#FDF3E7',
                                color: row.isCapex ? 'var(--blue)' : 'var(--amber)',
                              }}>
                                {row.isCapex ? 'CAPEX' : 'OPEX'}
                              </span>
                            ) : (
                          ...
                          {budgetYears.map(y => (
                            <td key={y} className="gf-td-r">{fmtEur(row.yearTotals[y] ?? 0)}</td>
                          ))}
                          <td className="gf-td-total">{fmtEur(row.total)}</td>
                        </tr>
                        {isExpanded && row.subRows.map(sub => (
                          <tr key={`${row.key}|${sub.planoId}`} className="bp-sub-row">
                            <td />
                            <td colSpan={2} style={{ paddingLeft: 28, color: 'var(--text2)', fontSize: 12 }}>
                              {sub.planoName || <span style={{ color: 'var(--text3)' }}>Sem plano</span>}
                            </td>
                            {budgetYears.map(y => (
                              <td key={y} className="gf-td-r" style={{ color: 'var(--text2)', fontSize: 12 }}>
                                {fmtEur(sub.yearTotals[y] ?? 0)}
                              </td>
                            ))}
                ...
                <tfoot>
                  <tr className="gf-total-row">
                    ...
                    {budgetYears.map(y => (
                      <td key={y} className="gf-td-r">{fmtEur(yearGrandTotals[y] ?? 0)}</td>
                    ))}
                    <td className="gf-td-r">{fmtEur(grandBudgetTotal)}</td>
                  </tr>
                </tfoot>
              </table>
            )}
