...
export default function Dashboard() {
  ...
  const kpis = useMemo(() => {
    ...
    const execution =
      activities.reduce((sum, a) => sum + (a.pct || 0), 0) / total;
    const executionObj =
      activities.reduce((sum, a) => sum + (a.pct_prev || 0), 0) / total;

    const concretization = completed / total;
    const concretizationObj = (completed + late) / total;
    const concretizationDate =
      completed + late > 0 ? completed / (completed + late) : 0;
  ...
  const groupedActivities = useMemo(() => {
    ...
    return sortedKeys.map((key) => {
      ...
      const execution =
        items.reduce((sum, a) => sum + (a.pct || 0), 0) / items.length;
      ...
      const n2Groups = Array.from(n2Map.keys())
        ...
        .map((n2Key) => {
          ...
          const n2Completed = n2Items.filter(
            (a) => a.status === "Concluído",
          ).length;
          const n2Late = n2Items.filter((a) => a.status === "Em atraso").length;
          const n2Execution =
            n2Items.reduce((sum, a) => sum + (a.pct || 0), 0) / n2Items.length;
  ...
  useEffect(() => {
    ...
    const onTimeData = groupedActivities.map(
      (g) => g.activities.length - g.completed - g.late,
    );
    ...
    if (donutCtx) {
      donutChartInstance.current = new Chart(donutCtx, {
        ...
        data: {
          ...
          datasets: [
            {
              data: [kpis.completed, kpis.onTime, kpis.late],
  ...
  const getStatusBadge = (completed: number, late: number, total: number) => {
    if (late > 0)
      return <span className="badge badge-late">{late} atraso</span>;
    if (completed === total)
  ...
  return (
    <section className="card">
      <h1>Dashboard</h1>
      <p>
        Filtro atual: <strong>{n0 || "Todos N0"}</strong> /{" "}
        <strong>{n1 || "Todos N1"}</strong>
      </p>
      ...
      <div className="kpi-grid">
        <div className="kpi-card">
          ...
          <div
            className={`kpi-value ${kpis.execution >= 75 ? "green" : kpis.execution < 40 ? "danger" : ""}`}
          >
        ...
        <div className="kpi-card">
          ...
          <div className={`kpi-value ${kpis.late > 0 ? "danger" : ""}`}>
            {kpis.late}
          </div>
        </div>
      </div>
      ...
      <div className="conc-grid">
        ...
        <div className="conc-card">
          <div className="conc-label">Real</div>
          <div className="conc-value">
            {formatPercent(kpis.concretization * 100)}
          </div>
        </div>
        <div className="conc-card obj">
          <div className="conc-label">Objectivo</div>
          <div className="conc-value">
            {formatPercent(kpis.concretizationObj * 100)}
          </div>
        </div>
        ...
        <div className="conc-card">
          <div className="conc-label">Real</div>
          <div className="conc-value">
            {formatPercent(kpis.concretizationDate * 100)}
          </div>
        </div>
        <div className="conc-card obj">
      ...
      <div className="charts-row" style={{ marginTop: "20px" }}>
        <div
          ...
          style={{ flex: "1.5", minHeight: "300px" }}
        >
        ...
        {group.n2Groups.map((n2Group) => (
          <div key={n2Group.n2} className="n2-row">
            ...
            <div className="n2-meta">
              <span className="fs-sm text-muted">
                {n2Group.activities.length} act.
              </span>
              {getStatusBadge(
                n2Group.completed,
                n2Group.late,
                n2Group.activities.length,
              )}
              <span className="fs-sm fw-bold">
                {formatPercent(n2Group.execution)}
              </span>
            </div>
            <div className="progress-wrap">
              <div
                ...
                style={{
                  width: `${Math.min(100, n2Group.execution).toFixed(1)}%`,
                }}
              ></div>
            </div>
          </div>
