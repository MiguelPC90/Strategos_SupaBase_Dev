...
export default function Activities() {
    ...
    const filteredActivities = useMemo(() => {
        ...
        return activities.filter((activity) => {
            ...
            return [activity.nome, activity.n1, activity.n0, activity.status]
    ...
    const rows = useMemo(() => {
        ...
        const hasMultiN0 = new Set(filteredActivities.map((a) => a.n0 || '')).size > 1;
        ...
        const kpi = (rows: Activity[]) => {
            ...
            const exe = total ? rows.reduce((sum, d) => sum + (d.pct || 0), 0) / total : 0;
            const exeObj = total ? rows.reduce((sum, d) => sum + (d.pct_prev || 0), 0) / total : 0;
        ...
        const localeSort = (values: string[]) => [...values].sort((a, b) => a.localeCompare(b, 'pt', { numeric: true, sensitivity: 'base' }));

        const rowsOut: ActivityRow[] = [];

        const n0Values = localeSort(Array.from(new Set(filteredActivities.map((d) => d.n0 || ''))).filter(Boolean));
        const renderN1Block = (n1Value: string, subset: Activity[]) => {
            ...
            const n2Values = localeSort(Array.from(new Set(subset.map((d) => d.n2 || ''))).filter(Boolean));
            n2Values.forEach((n2Value) => {
                ...
                const n3Values = localeSort(Array.from(new Set(subset2.map((d) => d.n3 || ''))).filter(Boolean));
                if (n3Values.length) {
        ...
        if (hasMultiN0) {
            ...
            n0ValuesSorted.forEach((n0Value) => {
                ...
                const n1Values = localeSort(Array.from(new Set(subset0.map((d) => d.n1 || ''))).filter(Boolean));
                n1Values.forEach((n1Value) => renderN1Block(n1Value, subset0.filter((d) => d.n1 === n1Value)));
            });
        } else {
            const n1Values = localeSort(Array.from(new Set(filteredActivities.map((d) => d.n1 || ''))).filter(Boolean));
            n1Values.forEach((n1Value) => renderN1Block(n1Value, filteredActivities.filter((d) => d.n1 === n1Value)));
        }
    ...
    const shouldRenderRow = (row: ActivityRow) => {
        ...
        if (row.type === 'n2') {
            return !collapsedKeys.has(`n1_${encodeURIComponent(row.n1)}`) && !collapsedKeys.has(`n0_${encodeURIComponent(row.n0)}`);
        }
        if (row.type === 'n3') {
            return (
                !collapsedKeys.has(`n2_${encodeURIComponent(row.n1)}_${encodeURIComponent(row.n2)}`) &&
        ...
        if (row.type === 'n4') {
            if (row.n3) {
                return (
                    !collapsedKeys.has(`n3_${encodeURIComponent(row.n1)}_${encodeURIComponent(row.n2)}_${encodeURIComponent(row.n3)}`) &&
                    !collapsedKeys.has(`n2_${encodeURIComponent(row.n1)}_${encodeURIComponent(row.n2)}`) &&
                    !collapsedKeys.has(`n1_${encodeURIComponent(row.n1)}`) &&
                    !collapsedKeys.has(`n0_${encodeURIComponent(row.n0)}`)
                );
        ...
        <section className="card act-page-card">
            <div className="act-toolbar">
                ...
                <div className="act-buttons">
                    <button className="btn" onClick={collapseAll}>Colapsar tudo</button>
                    <button className="btn" onClick={expandAll}>Expandir tudo</button>
                </div>
            </div>
            ...
            <div className="act-table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th scope="col" style={{ minWidth: '240px' }}>Designação</th>
                            <th scope="col" className="c" style={{ minWidth: '80px' }}>Estado</th>
                            <th scope="col" className="c" style={{ minWidth: '90px' }}>C / D / A</th>
                            <th scope="col" className="c" style={{ minWidth: '160px' }}>Exec. real / prev.</th>
                            <th scope="col" className="c" style={{ whiteSpace: 'nowrap' }}>Prazo</th>
                        </tr>
                    </thead>
                    <tbody>
                        {rows.length === 0 ? (
                            <tr>
                                <td colSpan={5} style={{ textAlign: 'center', color: 'var(--text2)', padding: '2rem' }}>
                                    ...
                                    <tr key={row.key} className={`act-${row.type} hover-row`}>
                                        <td className="act-name-cell">
                                            <div className="act-name-content">
                                                {hasChildren ? (
                                                    <button className="act-toggle" type="button" onClick={() => toggleRow(row.key)}>
                                                        {collapsed ? '▶' : '▼'}
                                                    </button>
                                                ) : null}
                                                <span className={row.type === 'n0' ? 'act-root-label' : undefined}>
                                                    {row.label || '—'}
                                                </span>
                                            </div>
                                        </td>
                                        <td className="c">
                                            {row.status ? <span className={getBadgeClass(row.status)}>{row.status}</span> : null}
                                        </td>
                                        <td className="c">
                                            {row.type === 'n4' ? null : (
                                                <span style={{ fontSize: '11px' }}>
                                                    <span style={{ color: 'var(--blue)' }}>{row.con}C</span>&nbsp;
                                                    <span style={{ color: 'var(--green)' }}>{row.dia}D</span>&nbsp;
                                                    <span style={{ color: 'var(--red)' }}>{row.atr}A</span>
                                                </span>
                                            )}
                                        </td>
                                        <td className="c">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', justifyContent: 'center' }}>
                                                <div style={{ width: '90px', position: 'relative', height: '10px', borderRadius: '3px', overflow: 'hidden', background: 'var(--bg3)' }}>
                                                    <div style={{ position: 'absolute', top: 0, left: 0, width: `${Math.min(100, Math.max(0, row.exeObj))}%`, height: '100%', background: 'var(--green)', opacity: .3, borderRadius: '3px' }}></div>
                                                    <div style={{ position: 'absolute', top: '1px', left: 0, width: `${Math.min(100, Math.max(0, row.exe))}%`, height: '8px', background: row.type === 'n4' ? 'var(--navy)' : 'var(--navy)', borderRadius: '3px' }}></div>
                                                </div>
                                                <span style={{ fontSize: '10px', minWidth: '50px', textAlign: 'left', color: 'var(--text2)' }}>
                                                    {formatPercent(row.exe)}<span style={{ color: 'var(--text3)' }}>/ {formatPercent(row.exeObj)}</span>
                                                </span>
                                            </div>
                                        </td>
                                        <td className="c act-deadline-cell" style={{ color: row.deadline && new Date(row.deadline) < new Date() ? 'var(--red)' : 'var(--text2)' }}>
